...
async function generateEmoji(prompt, env) {
  ...
  const createRes = await fetch('https://api.replicate.com/v1/predictions', {
    ...
    body: JSON.stringify({
      ...
      input: { prompt: emojiPrompt, apply_watermark: false }
    })
  });

  if (!createRes.ok) throw new Error('Failed to create prediction');
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (status.status === 'failed' || status.status === 'canceled') throw new Error('Generation failed');
  }
...
async function checkQuota(ip, env) {
  ...
  if (!data || data.date !== today) return { remaining: 3, date: today };
  return data;
}
